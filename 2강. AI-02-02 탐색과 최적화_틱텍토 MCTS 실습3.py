...
class TicTacToeBoard(TTTB, Node):  # TTTB의 속성들도 상속
    def find_children(self):  # 전체 가능한 move들 집합으로 반환
        if self.terminal:  # 게임이 끝나면 아무것도 하지 않음
            return set()
        return {  # 그렇지 않으면, 비어있는 곳에 각각 시도
            self.make_move(i) for i, value in enumerate(self.tup) if value is None
        }

    def find_random_child(self):  # 무작위로 move 선택
        if self.terminal:
...
def find_winner(tup):  # X가 이기면 True, O가 이기면 False, 미종료 상태면 None 반환
