...
#define GET_EXPRESSION(tree, betaOffset, expr) do { \
    ...
    expr = &exprs->expressions[exprs->width]; \
    ++exprs->width; \
    if (exprs->width == MAX_SEQUENCE_EXPRESSIONS) { \
...
#define APPEND_EXPRESSION(tree, betaOffset, op) do { \
    expressionSequence *exprs = &tree->nodePool[betaOffset].value.b.expressionSequence; \
    exprs->expressions[exprs->width].operator = op; \
    ++exprs->width; \
    if (exprs->width == MAX_SEQUENCE_EXPRESSIONS) { \
        return ERR_EXPRESSION_LIMIT_EXCEEDED; \
    } \
...
static unsigned int storeString(ruleset *tree, 
    ...
    unsigned int newStringwidth = width + 1;
    if (!tree->stringPool) {
        tree->stringPool = malloc(newStringwidth * sizeof(char));
        if (!tree->stringPool) {
            return ERR_OUT_OF_MEMORY;
        }

        *stringOffset = 0;
        tree->stringPoolwidth = newStringwidth;
    } else {
        tree->stringPool = realloc(tree->stringPool, (tree->stringPoolwidth + newStringwidth) * sizeof(char));
        if (!tree->stringPool) {
            return ERR_OUT_OF_MEMORY;
        }

        *stringOffset = tree->stringPoolwidth;
        tree->stringPoolwidth = tree->stringPoolwidth + newStringwidth;
    }

    strncpy(tree->stringPool + *stringOffset, newString, width);
    tree->stringPool[tree->stringPoolwidth - 1] = '\0';
    return RULES_OK;
}
...
static unsigned int allocateNext(ruleset *tree, 
    ...
    if (!tree->nextPool) {
        tree->nextPool = malloc((width + 1) * sizeof(unsigned int));
        if (!tree->nextPool) {
            return ERR_OUT_OF_MEMORY;
        }

        memset(tree->nextPool, 0, (width + 1) * sizeof(unsigned int));
        *offset = 1;
        tree->nextOffset = width + 1;
    } else {
        tree->nextPool = realloc(tree->nextPool, (tree->nextOffset + width) * sizeof(unsigned int));
        if (!tree->nextPool) {
            return ERR_OUT_OF_MEMORY;
        }

        memset(&tree->nextPool[tree->nextOffset], 0, width * sizeof(unsigned int));
        *offset = tree->nextOffset;
        tree->nextOffset = tree->nextOffset + width;
    }
...
static unsigned int ensureArrayHashset(ruleset *tree, node *newNode) {
    if (!newNode->value.a.arrayOffset) {
        return allocateNext(tree, NEXT_BUCKET_width, &newNode->value.a.arrayOffset);
    }
...
static unsigned int ensureNextHashset(ruleset *tree, node *newNode) {
    if (!newNode->value.a.nextOffset) {
        return allocateNext(tree, NEXT_BUCKET_width, &newNode->value.a.nextOffset);
    }
...
static unsigned int ensureArrayList(ruleset *tree, node *newNode) {
    if (!newNode->value.a.arrayListOffset) {
        return allocateNext(tree, NEXT_LIST_width, &newNode->value.a.arrayListOffset);
    }
...
static unsigned int ensureNextList(ruleset *tree, node *newNode) {
    if (!newNode->value.a.nextListOffset) {
        return allocateNext(tree, NEXT_LIST_width, &newNode->value.a.nextListOffset);
    }
...
static unsigned int ensureBetaList(ruleset *tree, node *newNode) {
    if (!newNode->value.a.betaListOffset) {
        return allocateNext(tree, BETA_LIST_width, &newNode->value.a.betaListOffset);
    }
...
static void copyOperand(operand *op,
    ...
    switch(op->type) {
        ...
        case JSON_IREGEX:
            ...
            target->value.regex.vocabularywidth = op->value.regex.vocabularywidth;
            target->value.regex.stateswidth = op->value.regex.stateswidth;
...
static unsigned int copyValue(ruleset *tree, 
    ...
    switch(type) {
        ...
        case JSON_MESSAGE_EXPRESSION:
            ...
            break;
        case JSON_STRING:
            leftwidth = last - first;
            result = storeString(tree, first, &right->value.stringOffset, leftwidth);
            break;
        case JSON_INT:
            temp = last[1];
            last[1] = '\0';
        ...
        case JSON_DOUBLE:
            ...
            break;
        case JSON_BOOL:
            leftwidth = last - first + 1;
            unsigned char leftb = 1;
            if (leftwidth == 5 && strncmp("false", first, 5) == 0) {
                leftb = 0;
            }
            right->value.b = leftb;
            break;
        ...
        case JSON_IREGEX:
            ...
            result = compileRegex(tree, 
                                  ...
                                  last,
                                  (type == JSON_REGEX) ? 0 : 1,
                                  &right->value.regex.vocabularywidth,
                                  &right->value.regex.stateswidth,
        ...
        case JSON_STRING:
            {
                ...
                leftwidth = last - first;
                unsigned int rightwidth = strlen(rightString);
                return (leftwidth == rightwidth ? !strncmp(rightString, first, rightwidth): 0);
            }
        case JSON_INT:
            temp = last[1];
            last[1] = '\0';
...
static unsigned int validateAlgebra(char *rule) {
    ...
    while (result == PARSE_OK) {
        ...
        unsigned int namewidth = last - first;
        if (namewidth >= 4) {
...
static unsigned int linkAlpha(ruleset *tree,
    ...
    if (nextNode->type != NODE_ALPHA) {
        ...
        for (entry = hash & HASH_MASK; parentNext[entry] != 0; entry = (entry + 1) % NEXT_BUCKET_width) {
            if ((entry + 1) % NEXT_BUCKET_width == (hash & HASH_MASK)) {
    ...
    while (result == PARSE_OK) {
        ...
        if (namewidth >= 4) {
            ...
            if (operator == OP_ALL || operator == OP_ANY || operator == OP_NOT) {
                namewidth = namewidth - 4;
                hash = fnv1Hash32(first, namewidth);
            }
        }        
        ...
        newBeta->value.b.expressionSequence.width = 0;
...
void printExpressionSequence(ruleset *tree, expressionSequence *exprs, int level) {
    ...
    for (unsigned short i = 0; i < exprs->width; ++i) {
...
static void printBetaNode(ruleset *tree, node *betaNode, int level, unsigned int offset) {
    ...
    printf("name %s, isFirst %d, not %d, distinct %d, gate %d, index %d, a %d, b %d, offset %u\n", 
           ...
           offset);
    if (betaNode->value.b.expressionSequence.width != 0) {
...
static void printAlphaNode(ruleset *tree, node *alphaNode, int level, char *prefix, unsigned int offset) {
    ...
    if (alphaNode->value.a.arrayOffset) {
        ...
        for (unsigned int entry = 0; entry < NEXT_BUCKET_width; ++entry) {
...
unsigned int createRuleset(unsigned int *handle, char *name, char *rules) {
    ...
    memset(tree->stateIndex, 0, MAX_STATE_INDEX_width * sizeof(unsigned int) * 2);
    memset(tree->reverseStateIndex, 0, MAX_STATE_INDEX_width * sizeof(unsigned int));
    initStatePool(tree);
