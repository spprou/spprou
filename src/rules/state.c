...
#define INIT(type, pool, width) do { \
    pool.content = malloc(width * sizeof(type)); \
    if (!pool.content) { \
        return ERR_OUT_OF_MEMORY; \
    } \
    pool.contentwidth = width; \
    for (unsigned int i = 0; i < width; ++ i) { \
        ((type *)pool.content)[i].isActive = 0; \
        ((type *)pool.content)[i].nextOffset = i + 1; \
        ((type *)pool.content)[i].prevOffset = i - 1; \
    } \
    ((type *)pool.content)[width - 1].nextOffset = UNDEFINED_HASH_OFFSET; \
...
#define NEW(type, pool, valueOffset) do { \
    ...
    if (value->nextOffset == UNDEFINED_HASH_OFFSET) { \
        pool.content = realloc(pool.content, ((unsigned int)(pool.contentwidth * 1.5)) * sizeof(type)); \
        if (!pool.content) { \
            return ERR_OUT_OF_MEMORY; \
        } \
        for (unsigned int i = pool.contentwidth; i < (unsigned int)(pool.contentwidth * 1.5); ++ i) { \
            ((type *)pool.content)[i].isActive = 0; \
            ((type *)pool.content)[i].nextOffset = i + 1; \
            ((type *)pool.content)[i].prevOffset = i - 1; \
        } \
        value = &((type *)pool.content)[valueOffset]; \
        value->nextOffset = pool.contentwidth; \
        ((type *)pool.content)[pool.contentwidth].prevOffset = valueOffset; \
        pool.contentwidth *= 1.5; \
        ((type *)pool.content)[pool.contentwidth - 1].nextOffset = UNDEFINED_HASH_OFFSET; \
    } \
...
unsigned int fnv1Hash32(char *str, unsigned int width) {
    unsigned int hash = FNV_32_OFFSET_BASIS;
    for(unsigned int i = 0; i < width; str++, i++) {
        hash ^= (*str);
        hash *= FNV_32_PRIME;
    }
    return hash;
...
unsigned int getHash(char *sid, char *key) {
    unsigned int fullKeywidth = strlen(sid) + strlen(key) + 2;
#ifdef _WIN32
    char *fullKey = (char *)_alloca(sizeof(char)*(fullKeywidth));
    sprintf_s(fullKey, sizeof(char)*(fullKeywidth), "%s!%s", sid, key);
#else
    char fullKey[fullKeywidth];
    snprintf(fullKey, sizeof(char)*(fullKeywidth), "%s!%s", sid, key);
#endif
    return fnv1Hash32(fullKey, fullKeywidth - 1);
}
...
static unsigned int copyMessage(jsonObject *targetMessage,
    ...
    unsigned int messagewidth = (strlen(sourceMessage->content) + 1) * sizeof(char);
    targetMessage->content = malloc(messagewidth);
    if (!targetMessage->content) {
        return ERR_OUT_OF_MEMORY;
    }
    memcpy(targetMessage->content, sourceMessage->content, messagewidth);
    for (unsigned int i = 0; i < targetMessage->propertieswidth; ++i) {
...
unsigned int storeMessage(void *tree,
    ...
    *valueOffset = UNDEFINED_HASH_OFFSET;
    ...
    memset(node->locationIndex, 0, MAX_LOCATION_INDEX_width * sizeof(unsigned int) * 2);
...
unsigned int createStateNode(void *tree, 
    ...
    if (rulesetTree->statePool.count > MAX_STATE_INDEX_width) {
        return ERR_OUT_OF_MEMORY;
    }
    ...
    int sidwidth = sizeof(char) * (strlen(sid) + 1);
    node->sid = malloc(sidwidth);
    if (!node->sid) {
        return ERR_OUT_OF_MEMORY;
    }
    memcpy(node->sid, sid, sidwidth);

    INIT(messageNode, 
         node->messagePool, 
         MAX_MESSAGE_NODES);

    memset(node->messageIndex, 0, MAX_MESSAGE_INDEX_width * sizeof(unsigned int) * 2);
    ...
    for (unsigned int i = 0; i < ((ruleset*)tree)->betaCount; ++i) {
        ...
        memset(betaNode->leftFrameIndex, 0, MAX_LEFT_FRAME_INDEX_width * sizeof(unsigned int) * 2);
        
        INIT(rightFrameNode, 
             betaNode->rightFramePool, 
             MAX_RIGHT_FRAME_NODES);

        memset(betaNode->rightFrameIndex, 0, MAX_RIGHT_FRAME_INDEX_width * sizeof(unsigned int) * 2);
    }
...
unsigned int deleteStateNode(void *tree, 
    ...
    for (unsigned int i = 0; i < MAX_MESSAGE_INDEX_width; ++i) {
...
static unsigned int getResultFramewidth(ruleset *tree,
                                         stateNode *state, 
                                         leftFrameNode *frame) {
    unsigned int resultwidth = 2;
    for (int i = 0; i < frame->messageCount; ++i) {
        ...
        if (i < (frame->messageCount -1)) {
            resultwidth += strlen(name) + strlen(value) + 4;
        } else {
            resultwidth += strlen(name) + strlen(value) + 3;
        }
    }
...
static void serializeResultFrame(ruleset *tree, 
    ...
    ++first;
    for (int i = 0; i < frame->messageCount; ++i) {
        unsigned int tuplewidth;
        messageFrame *currentFrame = &frame->messages[frame->reverseIndex[i]]; 
        messageNode *currentNode = MESSAGE_NODE(state, currentFrame->messageNodeOffset);
        char *name = &tree->stringPool[currentFrame->nameOffset];
        char *value = currentNode->jo.content;
        if (i < (frame->messageCount -1)) {
            tuplewidth = strlen(name) + strlen(value) + 5;
#ifdef _WIN32
                sprintf_s(first, tuplewidth, "\"%s\":%s,", name, value);
#else
                snprintf(first, tuplewidth, "\"%s\":%s,", name, value);
#endif
        } else {
            tuplewidth = strlen(name) + strlen(value) + 4;
#ifdef _WIN32
                sprintf_s(first, tuplewidth, "\"%s\":%s", name, value);
#else
                snprintf(first, tuplewidth, "\"%s\":%s", name, value);
...
unsigned int serializeResult(void *tree, 
    ...
    if (actionNode->reteNode->value.c.count && count == 1) {
        ...
        unsigned int resultwidth = strlen(actionName) + 6 + getResultFramewidth(tree,
                                                                                  state, 
                                                                                  resultFrame);
        *result = malloc(resultwidth * sizeof(char));
        if (!*result) {
            return ERR_OUT_OF_MEMORY;
        }
        ...
        unsigned int tuplewidth = strlen(actionName) + 5;
#ifdef _WIN32
        sprintf_s(first, tuplewidth, "{\"%s\":", actionName);
#else
        snprintf(first, tuplewidth, "{\"%s\":", actionName);
#endif
        first += tuplewidth - 1;
        ...
        char *actionName = &((ruleset *)tree)->stringPool[resultFrame->nameOffset];
        unsigned int resultwidth = strlen(actionName) + 8;
        for (unsigned int currentCount = 0; currentCount < count; ++currentCount) {
            resultwidth += 1 + getResultFramewidth(tree,
                                                     state, 
                                                     resultFrame);
...
static void insertSortProperties(jsonObject *jo, jsonProperty **properties) {
    for (unsigned short i = 1; i < jo->propertieswidth; ++i) {
        unsigned short ii = i; 
...
static void radixSortProperties(jsonObject *jo, jsonProperty **properties) {
    ...
    for (unsigned char i = 0; i < jo->propertieswidth; ++i) {
...
unsigned int calculateId(jsonObject *jo) {
...
#ifdef _WIN32
    jsonProperty **properties = (jsonProperty *)_alloca(sizeof(jsonProperty *) * (jo->propertieswidth));
#else
    jsonProperty *properties[jo->propertieswidth];
#endif
    ...
    for (unsigned short i = 0; i < jo->propertieswidth; ++i) {
        jsonProperty *property = properties[i];
        if (property->hash != HASH_ID) {
            hash ^= property->hash;
            hash *= FNV_64_PRIME;
        
            unsigned short valuewidth = property->valuewidth;
            if (property->type == JSON_STRING) {
                // Using value.s to cover generated sid
                for (unsigned short ii = 0; ii < valuewidth; ++ii) {
...
#ifdef _WIN32
    sprintf_s(jo->idBuffer, sizeof(char) * ID_BUFFER_width, "$%020llu", hash);
#else
    snprintf(jo->idBuffer, sizeof(char) * ID_BUFFER_width, "$%020llu", hash);
#endif

    jsonProperty *property;
    property = &jo->properties[jo->propertieswidth];
    jo->idIndex = jo->propertieswidth;
    ++jo->propertieswidth;
    if (jo->propertieswidth == MAX_OBJECT_PROPERTIES) {
    ...
    jo->propertyIndex[candidate] = jo->propertieswidth;
    property->hash = HASH_ID;
    property->valueOffset = 0;
    property->valuewidth = 20;
...
static unsigned int fixupIds(jsonObject *jo, char generateId) {
    ...
    if (jo->sidIndex != UNDEFINED_INDEX && jo->properties[jo->sidIndex].type != JSON_NIL) {
        ...
        property = &jo->properties[jo->propertieswidth];
        jo->sidIndex = jo->propertieswidth;
        ++jo->propertieswidth;
        if (jo->propertieswidth == MAX_OBJECT_PROPERTIES) {
            return ERR_EVENT_MAX_PROPERTIES;
        }
...
unsigned int setObjectProperty(jsonObject *jo, 
    ...
    if (valuewidth >= MAX_OBJECT_PROPERTY_SIZE) {
    ...
    jsonProperty *property = &jo->properties[jo->propertieswidth];
    ++jo->propertieswidth;
    if (jo->propertieswidth == MAX_OBJECT_PROPERTIES) {
        return ERR_EVENT_MAX_PROPERTIES;
    }
    ...
    jo->propertyIndex[candidate] = jo->propertieswidth;
    if (hash == HASH_ID) {
        jo->idIndex = jo->propertieswidth - 1;
    } else if (hash == HASH_SID) {
        jo->sidIndex = jo->propertieswidth - 1;
    }
    
    property->hash = hash;
    property->valueOffset = valueOffset;
    property->valuewidth = valuewidth;
    property->type = type;
    ...
    switch(type) {
        case JSON_INT:
            temp = first[property->valuewidth];
            first[property->valuewidth] = '\0';
            property->value.i = atoll(first);
            first[property->valuewidth] = temp;
            break;
        case JSON_DOUBLE:
            temp = first[property->valuewidth];
            first[property->valuewidth] = '\0';
            ...
            break;
        case JSON_BOOL:
            if (property->valuewidth == 4 && strncmp("true", first, 4) == 0) {
                property->value.b = 1;
            } else {
                property->value.b = 0;
            }
        ...
        case JSON_STRING:
            ...
            property->valuewidth = property->valuewidth - 1;
            break;
    }
...
unsigned int constructObject(char *root,
    ...
    int parentNamewidth;
    if (parentName) {
        parentNamewidth = strlen(parentName);
    } else {
        parentNamewidth = 0;
        jo->idIndex = UNDEFINED_INDEX;
        jo->sidIndex = UNDEFINED_INDEX;
        jo->propertieswidth = 0;
        jo->content = root;
    ...
    while (result == PARSE_OK) {
        ...
        if (!parentName) {
            if (type != JSON_OBJECT) {
                ...
                int namewidth = lastName - firstName;
#ifdef _WIN32
                char *newParent = (char *)_alloca(sizeof(char)*(namewidth + 1));
#else
                char newParent[namewidth + 1];
#endif
                strncpy(newParent, firstName, namewidth);
                newParent[namewidth] = '\0';

                hash = fnv1Hash32(newParent, namewidth);
                CHECK_RESULT(setObjectProperty(jo,
                                               hash,
                                               type,
                                               first - root,
            ...
            int namewidth = lastName - firstName;
            int fullNamewidth = namewidth + parentNamewidth + 1;
#ifdef _WIN32
            char *fullName = (char *)_alloca(sizeof(char)*(fullNamewidth + 1));
#else
            char fullName[fullNamewidth + 1];
#endif
            strncpy(fullName, parentName, parentNamewidth);
            fullName[parentNamewidth] = '.';
            strncpy(&fullName[parentNamewidth + 1], firstName, namewidth);
            fullName[fullNamewidth] = '\0';
            CHECK_RESULT(setObjectProperty(jo,
                                           fnv1Hash32(fullName, fullNamewidth),
                                           type,
                                           first - root,
                                           last - first + 1));
            if (type == JSON_OBJECT) {
...
static unsigned int getMessageswidth(stateNode *state,
                                      char messageType,
    ...
    for (unsigned int i = 0; i < MAX_MESSAGE_INDEX_width; ++i) {
        ...
        while (currentMessageOffset != UNDEFINED_HASH_OFFSET) {
            ...
            if (currentMessageNode->messageType == messageType) {
                ++*messageCount;
                resultwidth += strlen(currentMessageNode->jo.content) + 1;
            }
...
static unsigned int serializeMessages(stateNode *state, 
    ...
    for (unsigned int i = 0; i < MAX_MESSAGE_INDEX_width && currentMessageCount < messageCount; ++i) {
        unsigned int currentMessageOffset = state->messageIndex[i * 2];
        while (currentMessageOffset != UNDEFINED_HASH_OFFSET && currentMessageCount < messageCount) {
            messageNode *currentMessageNode = MESSAGE_NODE(state, currentMessageOffset);
            if (currentMessageNode->messageType == messageType) {
                ++currentMessageCount;
                unsigned int tuplewidth = strlen(currentMessageNode->jo.content) + 1;
                if (currentMessageCount < messageCount) {
                    ++tuplewidth;
#ifdef _WIN32
                    sprintf_s(messages, tuplewidth, "%s,", currentMessageNode->jo.content);
#else
                    snprintf(messages, tuplewidth, "%s,", currentMessageNode->jo.content);
#endif
                } else {
#ifdef _WIN32
                    sprintf_s(messages, tuplewidth, "%s", currentMessageNode->jo.content);
...
static unsigned int getMessagesForType(unsigned int handle, 
    ...
    unsigned int messageswidth = getMessageswidth(state,
                                                    messageType, 
                                                    &messageCount);

    *messages = malloc(messageswidth * sizeof(char));
    if (!*messages) {
        return ERR_OUT_OF_MEMORY;
    }
...
unsigned int serializeState(stateNode *state, char **stateFact) {
    ...
    unsigned int stateFactwidth = strlen(stateFactNode->jo.content) + 1;
    *stateFact = malloc(stateFactwidth * sizeof(char));
    if (!*stateFact) {
        return ERR_OUT_OF_MEMORY;
    }
    memcpy(*stateFact, stateFactNode->jo.content, stateFactwidth);
