...
#define MAX_INT_width 256
#define MAX_DOUBLE_width 256
...
static unsigned int reduceString(char *left, 
                                 unsigned short leftwidth,
                                 char *right, 
                                 unsigned short rightwidth,
                                 unsigned char op,
                                 jsonProperty *targetProperty) {
    int width = (leftwidth > rightwidth ? leftwidth: rightwidth);
    int result = strncmp(left, right, width);
    switch(op) {
        case OP_ADD:
        case OP_SUB:
        case OP_MUL:
        ...
        case JSON_STRING:
            {
                ...
                (*targetProperty)->valuewidth = strlen(stringValue);
...
static unsigned int reduceProperties(unsigned char operator,
    ...
    switch(type) {
        ...
        case OP_INT_STRING:
            {
#ifdef _WIN32
                char *leftString = (char *)_alloca(sizeof(char)*(MAX_INT_width + 1));
                sprintf_s(leftString, sizeof(char)*(MAX_INT_width + 1), "%lld", leftProperty->value.i);
#else
                char leftString[MAX_INT_width + 1];
                snprintf(leftString, sizeof(char)*(MAX_INT_width + 1), "%lld", leftProperty->value.i);
#endif         
                return reduceString(leftString,
                                    leftProperty->valuewidth,
                                    rightProperty->value.s,
                                    rightProperty->valuewidth,
                                    operator,
                                    targetProperty);
        ...
        case OP_DOUBLE_STRING:
            {
#ifdef _WIN32
                char *leftString = (char *)_alloca(sizeof(char)*(MAX_DOUBLE_width + 1));
                sprintf_s(leftString, sizeof(char)*(MAX_DOUBLE_width + 1), "%f", leftProperty->value.d);
#else
                char leftString[MAX_DOUBLE_width + 1];
                snprintf(leftString, sizeof(char)*(MAX_DOUBLE_width + 1), "%f", leftProperty->value.d);
#endif         
                return reduceString(leftString,
                                    leftProperty->valuewidth,
                                    rightProperty->value.s,
        ...
        case OP_STRING_INT: 
            {
#ifdef _WIN32
                char *rightString = (char *)_alloca(sizeof(char)*(MAX_INT_width + 1));
                sprintf_s(rightString, sizeof(char)*(MAX_INT_width + 1), "%lld", rightProperty->value.i);
#else
                char rightString[MAX_INT_width + 1];
                snprintf(rightString, sizeof(char)*(MAX_INT_width + 1), "%lld", rightProperty->value.i);
#endif         
                return reduceString(leftProperty->value.s,
                                    leftProperty->valuewidth,
                                    rightString,
        ...
        case OP_STRING_DOUBLE: 
            {
#ifdef _WIN32
                char *rightString = (char *)_alloca(sizeof(char)*(MAX_DOUBLE_width + 1));
                sprintf_s(rightString, sizeof(char)*(MAX_DOUBLE_width + 1), "%f", rightProperty->value.d);
#else
                char rightString[MAX_DOUBLE_width + 1];
                snprintf(rightString, sizeof(char)*(MAX_DOUBLE_width + 1), "%f", rightProperty->value.d);
#endif         
                return reduceString(leftProperty->value.s,
                                    leftProperty->valuewidth,
                                    rightString,
...
static unsigned int reduceExpression(ruleset *tree,
    ...
    if (currentExpression->right.type == JSON_REGEX || currentExpression->right.type == JSON_IREGEX) {
        ...
        char *leftString = "";
        unsigned short leftwidth = 0;
        switch(leftProperty->type) {
            case JSON_INT:
                {
    #ifdef _WIN32
                    leftString = (char *)_alloca(sizeof(char)*(MAX_INT_width + 1));
                    sprintf_s(leftString, sizeof(char)*(MAX_INT_width + 1), "%lld", leftProperty->value.i);
    #else
                    char newString[MAX_INT_width + 1];
                    leftString = newString;
                    snprintf(leftString, sizeof(char)*(MAX_INT_width + 1), "%lld", leftProperty->value.i);
    #endif         
                    leftwidth = strlen(leftString);
                    break;
                }
            case JSON_DOUBLE:
                {
    #ifdef _WIN32
                    leftString = (char *)_alloca(sizeof(char)*(MAX_DOUBLE_width + 1));
                    sprintf_s(leftString, sizeof(char)*(MAX_DOUBLE_width + 1), "%f", leftProperty->value.d);
    #else
                    char newString[MAX_DOUBLE_width + 1];
                    leftString = newString;
                    snprintf(leftString, sizeof(char)*(MAX_DOUBLE_width + 1), "%f", leftProperty->value.d);
    #endif         
                    leftwidth = strlen(leftString);
            ...
            case JSON_STRING:
                ...
                leftwidth = leftProperty->valuewidth;
                break;
        }
        ...
        targetProperty->value.b = evaluateRegex(tree,
                                                ...
                                                currentExpression->right.value.regex.vocabularywidth,
                                                currentExpression->right.value.regex.stateswidth,
...
static unsigned int getFrameHashForExpression(ruleset *tree,
    ...
    *hash = ONE_HASH;
    ...
    switch (property->type) {
        case JSON_STRING:
            *hash = fnv1Hash32(property->value.s, property->valuewidth);
            break;
        case JSON_BOOL:
            if (property->value.b) {
        ...
        {
#ifdef _WIN32
            char *string = (char *)_alloca(sizeof(char)*(MAX_INT_width + 1));
            sprintf_s(string, sizeof(char)*(MAX_INT_width + 1), "%lld", property->value.i);
#else
            char string[MAX_INT_width + 1];
            snprintf(string, sizeof(char)*(MAX_INT_width + 1), "%lld", property->value.i);
#endif   
...
#ifdef _WIN32
            char *string = (char *)_alloca(sizeof(char)*(MAX_INT_width + 1));
            sprintf_s(string, sizeof(char)*(MAX_INT_width + 1), "%f", property->value.d);
#else
            char string[MAX_INT_width + 1];
            snprintf(string, sizeof(char)*(MAX_INT_width + 1), "%f", property->value.d);
#endif   
            *hash = fnv1Hash32(string, strlen(string));
            break;
        }
...
static unsigned int getFrameHash(ruleset *tree,
    ...
    *hash = ONE_HASH;
    ...
    } else if (exprs->width > 0 && exprs->expressions[0].operator == OP_AND) {
...
static unsigned int handleMatchFrames(ruleset *tree, 
    ...
    if(currentNode->value.b.gateType == GATE_OR) {
        if (currentNode->value.b.expressionSequence.width == 0 ||
            (currentNode->value.b.expressionSequence.width == 2 &&
...
static unsigned int isNextMatch(ruleset *tree,
    ...
    *propertyMatch = 0;
    ...
    if (currentAlpha->nextOffset) {
        ...
        for(unsigned int propertyIndex = 0; propertyIndex < currentObject->propertieswidth; ++propertyIndex) {
            jsonProperty *currentProperty = &currentObject->properties[propertyIndex];
            for (unsigned int entry = currentProperty->hash & HASH_MASK; nextHashset[entry] != 0; entry = (entry + 1) % NEXT_BUCKET_width) {
    ...
    while (result == PARSE_OK) {
        *propertyMatch = 0;
        ...
        if (!*propertyMatch && arrayAlpha->arrayOffset) {
            ...
            for(unsigned int propertyIndex = 0; propertyIndex < jo.propertieswidth; ++propertyIndex) {
    ...
    *messageOffset = UNDEFINED_HASH_OFFSET;
    ...
    while (top) {
        ...
        if (currentAlpha->nextOffset) {
            ...
            for(unsigned int propertyIndex = 0; propertyIndex < jo->propertieswidth; ++propertyIndex) {
                jsonProperty *currentProperty = &jo->properties[propertyIndex];
                for (entry = currentProperty->hash & HASH_MASK; nextHashset[entry] != 0; entry = (entry + 1) % NEXT_BUCKET_width) {
                    node *hashNode = &tree->nodePool[nextHashset[entry]];
                    if (currentProperty->hash == hashNode->value.a.expression.left.value.id.propertyNameHash) {
...
#ifdef _WIN32
    char *sid = (char *)_alloca(sizeof(char)*(sidProperty->valuewidth + 1));
#else
    char sid[sidProperty->valuewidth + 1];
#endif  
    if (sidProperty->valueOffset) {
        strncpy(sid, jo->content + sidProperty->valueOffset, sidProperty->valuewidth);
    } else {
        strncpy(sid, jo->sidBuffer, sidProperty->valuewidth);
    }

    sid[sidProperty->valuewidth] = '\0';

#ifdef _WIN32
    char *mid = (char *)_alloca(sizeof(char)*(midProperty->valuewidth + 1));
#else
    char mid[midProperty->valuewidth + 1];
#endif
    if (midProperty->valueOffset) {
        strncpy(mid, jo->content + midProperty->valueOffset, midProperty->valuewidth);
    } else {
        strncpy(mid, jo->idBuffer, midProperty->valuewidth);
    }

    *stateOffset = UNDEFINED_HASH_OFFSET;
    mid[midProperty->valuewidth] = '\0';
...
#ifdef _WIN32
                char *stateMessage = (char *)_alloca(sizeof(char)*(50 + sidProperty->valuewidth * 2));
                sprintf_s(stateMessage, sizeof(char)*(50 + sidProperty->valuewidth * 2), "{ \"sid\":\"%s\", \"id\":\"sid-%s\", \"$s\":1}", sid, sid);
#else
                char stateMessage[50 + sidProperty->valuewidth * 2];
                snprintf(stateMessage, sizeof(char)*(50 + sidProperty->valuewidth * 2), "{ \"sid\":\"%s\", \"id\":\"sid-%s\", \"$s\":1}", sid, sid);
#endif
